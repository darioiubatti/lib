...
export default function BooksFieldsImport({ books, onClose, onUpdate }) {
    ...
    reader.onload = async (e) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          const book = books.find(b => b.id_univoco === idUnivoco);
          if (!book) {
            addLog(`❌ ${idUnivoco}: Libro non trovato`);
          ...
          if (rowData['prima edizione'] !== undefined) {
            updateData.prima_edizione = rowData['prima edizione'].toLowerCase() === 'true' || rowData['prima edizione'] === '1' || rowData['prima edizione'].toLowerCase() === 'sì';
            changes.push('prima_edizione');
          }
          
          if (rowData['autografato'] !== undefined) {
            updateData.autografato = rowData['autografato'].toLowerCase() === 'true' || rowData['autografato'] === '1' || rowData['autografato'].toLowerCase() === 'sì';
            changes.push('autografato');
          }
          
          if (rowData['sovraccoperta'] !== undefined) {
            updateData.sovraccoperta = rowData['sovraccoperta'].toLowerCase() === 'true' || rowData['sovraccoperta'] === '1' || rowData['sovraccoperta'].toLowerCase() === 'sì';
            changes.push('sovraccoperta');
          }

          await base44.entities.Book.update(book.id, updateData);
          addLog(`✅ ${idUnivoco}: Aggiornato (${changes.join(', ')})`);
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto bg-white">
        <DialogHeader className="bg-white">
          <DialogTitle>Importa Campi Libri da CSV</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4 bg-white">
          <Alert className="bg-white">
            ...
            <AlertDescription>
              ...
              <ul className="list-disc list-inside text-sm space-y-1">
                <li>Colonne: <code>id univoco, prima edizione, autografato, sovraccoperta</code></li>
                <li>Valori booleani: true/false, 1/0, sì/no</li>
          ...
          <div className="border-2 border-dashed rounded-lg p-8 text-center">
            <input
              ...
              className="hidden"
              id="csv-upload-fields"
            />
            <label
              htmlFor="csv-upload-fields"
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Aggiorna i campi prima edizione, autografato e sovraccoperta
                </p>
              </div>
            </label>
          </div>
                ...
                <Alert className="bg-green-50 border-green-200">
                  ...
                  <AlertDescription>
                    <p className="font-semibold text-green-800 mb-2">
                      {results.success.length} libri aggiornati con successo
                    </p>
                ...
                <Alert className="bg-amber-50 border-amber-200">
                  ...
                  <AlertDescription>
                    <p className="font-semibold text-amber-800 mb-2">
                      {results.notFound.length} libri non trovati
                    </p>
